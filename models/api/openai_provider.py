...
class OpenAIProvider(BaseModel):
    def __init__(self, model: str = "gpt-4o", role: str = "api_analyst"):
        super().__init__(name=model, role=role, description=f"OpenAI {model}")
        self.model = model
        self.api_key = os.getenv("OPENAI_API_KEY", "")
        self.client = AsyncOpenAI(api_key=self.api_key) if AVAILABLE and self.api_key else None
