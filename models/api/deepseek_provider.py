...
class DeepSeekProvider(BaseModel):
    def __init__(self, model: str = "deepseek-chat", role: str = "api_analyst"):
        super().__init__(name=model, role=role, description=f"DeepSeek {model}")
        self.model = model
        self.api_key = os.getenv("DEEPSEEK_API_KEY", "")
        self.client = (
            AsyncOpenAI(api_key=self.api_key, base_url="https://api.deepseek.com/v1")
